...
#define SRA(_a, _b, _len) (_BITS((_SIGN(_a, _len) ^ _a), _len) >> _b ^ _SIGN(_a, _len)) // a portable SRA
    ...
    INSTPAT("0000000 ????? ????? 001 ????? 00110 11", slliw, I, R(dest) = SEXT(BITS(src1 << imm, 31, 0), 32));
    INSTPAT("0000000 ????? ????? 001 ????? 01110 11", sllw, R,
            R(dest) = SEXT(BITS(src1 << BITS(src2, 4, 0), 31, 0), 32));
    ...
    INSTPAT("0000000 ????? ????? 101 ????? 00110 11", srliw, I, R(dest) = SEXT(BITS(src1, 31, 0) >> imm, 32));
